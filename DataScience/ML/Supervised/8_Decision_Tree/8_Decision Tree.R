...
cm <- table(iris$Species,iris$Species_Predicted)
misClassError <- mean(iris$Species!=iris$Species_Predicted)
print(paste('Accuracy =',1-misClassError))
library(caret)
confusionMatrix(cm)
##############################################################################
...
# Step 2:Train model with decision tree using rpart function of rpart library
